<script lang="ts">
  ...
  export let type: 'text' | 'textarea' | 'select' | 'date' | 'time' | 'datetime-local' | 'number' =
    'text';
...
<label class="form-control">
  ...
  {#if type === 'textarea'}
    <textarea class="textarea textarea-bordered h-24" {name} {placeholder} bind:value />
  {:else if type === 'select'}
    <select class="select select-bordered" {name} {placeholder} bind:value>
      <slot />
    </select>
  {:else if type === 'date'}
    <input class="input input-bordered" type="date" {name} {placeholder} bind:value />
  {:else if type === 'datetime-local'}
    <input class="input input-bordered" type="datetime-local" {name} {placeholder} bind:value />
  {:else if type === 'number'}
